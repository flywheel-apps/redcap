...
def process_files(container, files, rc_rec_field, fw_rec_val):
    ...
    file_calls = get_calls_for_containers(file_containers,
                                          ...
                                          fw_rec_val,
                                          'file')
    file_map = level_export_template(rc_rec_field, fw_rec_val, file_map, 'file')
    ...
    for container in containers:
        ...
        if all_container_map:
            ...
            if all_container_files:
                calls.append(process_files(container, all_container_files, rc_rec_field, fw_rec_val))
        ...
        if container_map:
            ...
            temp_map = level_export_template(rc_rec_field, fw_rec_val, container_maps, level)
...
def level_map(project, subject_map, session_map, acquisition_map, rc_rec_field=None, fw_rec_val = None):
...
def map_container_and_subcontainers(dict_in, level, parent_container, prev_rc_id, prev_id_level, prev_fw_key, prev_fw_id):
    ...
    for child in children:
        ...
        if next_level is None:
            ...
            children_call = map_container_and_subcontainers(dict_in, next_level, child, rc_id, id_level,
                                    fw_key, fw_id)
...
def level_map2(dict_in, project):
    ...
    full_call = map_container_and_subcontainers(dict_in, level, parent_container, prev_rc_id, prev_id_level,
...
fw=flywheel.Client(os.environ['COVID_API'])

yaml_file_path = '/Users/davidparker/Documents/Flywheel/Clients/CCIC/REDCap/Flywheel2Redcap_template.yaml'
with open(yaml_file_path) as file:
    dict_in = yaml.full_load(file)
    file.close()

project = '5f96e95d8b974cba06cd67c6' # Centura
# project = '5f7b62777f0eb828e102b866'
project = fw.get(project)
calls = level_map2(dict_in, project)
calls = collapse_calls_by_record(calls, 'subject_rc_id')

api_token = '28FA2B55EEF6AE7E4BEA58DA2B6EC951'
rc_url = 'https://redcap.uahs.arizona.edu/api/'
project = Project(rc_url, api_token)

rr_map = get_redcap_mc_map(project)
log.debug(rr_map)
